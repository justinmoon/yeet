...
import type { Attestation, KeyExchangeResponse, MapleConfig } from "./types";
...
async function establishSession(config: MapleConfig): Promise<Attestation> {
  logger.debug("Establishing secure session with Maple", {
  ...
  const attestationBase64 = await fetchAttestationDocument(
    config.apiUrl,
    nonce,
  );
                ...
                if (data === "[DONE]") {
                  ...
                  try {
                    ...
                    try {
                      ...
                      controller.enqueue(
                        encoder.encode(`data: ${decrypted}\n`),
                      );
                    }
                  } catch (error) {
                    logger.error("Failed to decrypt SSE chunk", {
                      error: String(error),
                    });
...
> {
  ...
  return async (
    ...
    try {
      ...
      logger.error("Maple fetch error", {
        error: String(error),
        url: input.toString(),
      });
      throw error;
