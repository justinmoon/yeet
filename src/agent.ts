  ...
  try {
    ...
    for await (const chunk of result.fullStream) {
      ...
      if (chunk.type === "tool-call") {
        // Log EVERYTHING in the chunk
        console.error("[AGENT] Tool call chunk keys:", Object.keys(chunk))
        console.error("[AGENT] Full chunk:", JSON.stringify(chunk, null, 2))
        
        onToolCall?.(chunk.toolName)
        
        // Get args from chunk.input (that's where they actually are!)
        const args = chunk.input || chunk.args || {}
        
        console.error("[AGENT] Extracted args:", JSON.stringify(args, null, 2))
