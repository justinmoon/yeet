...
const TEST_DIRS = ["/tmp/workflow-coder", "/tmp/workflow-reviewer"];
...
describe("Simple Workflow", () => {
  test("should run coder then reviewer", async () => {
    const task =
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        actor.stop();
        reject(new Error("Workflow timed out after 2 minutes"));
  ...
  test("should handle multiple steps in implementation", async () => {
    const task =
