...
async function runFizzBuzzTest(config: Config, testName: string) {
  ...
  console.log(
    `Model: ${config.activeProvider === "opencode" ? config.opencode.model : config.maple?.model}`,
  );
    ...
    for await (const event of runAgent(messages, config)) {
      if (event.type === "text") {
        ...
        if (typeof event.result === "string") {
          ...
          if (event.name && event.name === "bash" && stdout) {
            finalOutput = stdout;
          }
...
test(
  "FizzBuzz E2E with OpenCode Zen",
  async () => {
...
test(
  "FizzBuzz E2E with Maple AI - Qwen",
  async () => {
    const config = await loadConfig();
...
test(
  "FizzBuzz E2E with Maple AI - Llama",
  async () => {
    const config = await loadConfig();
    ...
    await runFizzBuzzTest(
      mapleConfig,
      "FizzBuzz with Maple AI (Llama 3.3 70B)",
    );
  },
  TEST_TIMEOUT,
);
...
test(
  "FizzBuzz E2E with Maple AI - GPT OSS 120B",
  async () => {
    const config = await loadConfig();
...
test(
  "FizzBuzz E2E with Maple AI - DeepSeek R1",
  async () => {
    const config = await loadConfig();
    ...
    await runFizzBuzzTest(
      mapleConfig,
      "FizzBuzz with Maple AI (DeepSeek R1 671B)",
    );
  },
  TEST_TIMEOUT,
);
...
test(
  "FizzBuzz E2E with Maple AI - DeepSeek V3.1",
  async () => {
    const config = await loadConfig();
    ...
    await runFizzBuzzTest(
      mapleConfig,
      "FizzBuzz with Maple AI (DeepSeek V3.1 Terminus)",
    );
  },
  TEST_TIMEOUT,
);
...
test(
  "FizzBuzz E2E with Maple AI - GPT-OSS",
  async () => {
    const config = await loadConfig();
...
test(
  "FizzBuzz E2E with Maple AI - Gemma 3",
  async () => {
    const config = await loadConfig();
