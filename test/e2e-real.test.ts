...
test(
  "E2E with REAL API - bash tool with clean output",
  async () => {
    ...
    const suspiciousLines = lines.filter((line) => {
      ...
      const hasMixedFiles =
        /[a-z][A-Z]/.test(line) && /attach|agents\.md|bun\.lock/i.test(line);
      ...
      const hasFileContent = /attach|bun\.lock|CHANGELOG|agents\.md/i.test(
        line,
      );
